...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Admin check logic
    const adminEmails = JSON.parse(localStorage.getItem("asident_admin_emails") || '["rainandanabilatu@gmail.com"]');
    const role = adminEmails.includes(email) ? "admin" : "pasien";
        ...
        <div className="grid grid-cols-1 md:grid-cols-5">
          ...
          <div className="md:col-span-3 p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button 
                type="button"
                onClick={() => onLogin("User Google", "pasien", "google-user@gmail.com")}
                className="flex w-full items-center justify-center gap-3 rounded-2xl border-2 border-slate-100 bg-white py-4 text-sm font-bold text-slate-700 transition-all hover:bg-slate-50 active:scale-[0.98] shadow-sm"
              >
